        ...
        with torch.no_grad():
            ...
            if labels == None:
                labels = torch.randint(
                    0, 9, (self.args.n_samples,), device=self.args.device
                )
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                t = n_steps - t_ - 1
                t = torch.full(
                    (self.args.n_samples,), t, device=z_t.device, dtype=torch.long
                )
                ...
                eps_tilde_t = (1.0 + cfg_scale) * self.eps_model(
                    z_t, labels
                ) - cfg_scale * self.eps_model(z_t)

                # TODO: Get x_t then sample z_t from the reverse process according to 4. and 5. in Algo 2.
                sigma_lambda_t = self.diffusion.sigma_lambda(lambda_t)
                alpha_lambda_t = self.diffusion.alpha_lambda(lambda_t)
                x_t = (z_t - sigma_lambda_t * eps_tilde_t) / alpha_lambda_t
                ...
                if self.args.nb_save is not None and t_ in saving_steps:
                    print(
                        f"Showing/saving samples from epoch {self.current_epoch} with labels: {labels.tolist()}"
                    )
                    show_save(
                        x_t,
                        labels,
